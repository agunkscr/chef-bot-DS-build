  ...
  switch (mode) {
    ...
    case "adaptive":
      return {
        ...
        ...base,
        maxSlippageBps: base.maxSlippageBps || 400,
        minVolume24h: base.minVolume24h || 0,
        signalsRequired: [],
        signalsForbidden: [],
        minGrade: 1,
        maxTotalPositions: 4,
...
> {
  ...
  const candidates = ingredients.filter((ing) => {
    ...
    // Cooldown jual
    if (isRecentlySold(ing.id, cooldownMinutes)) {
      return false;
    }
  ...
  if (cfg.mode === "adaptive") {
    ...
    const scored = enriched.map((item) => {
      ...
      const rank = item.token.signal_rank ?? 10;

      // Bobot sinyal bazaar
      if (tokenSignals.includes("LOW_VALUATION")) score += 3;
      if (tokenSignals.includes("VOLUME_SPIKE")) score += 3;
      if (tokenSignals.includes("MOMENTUM_12H")) score += 1;

      // Bobot peringkat: rank 1 → +5, rank 10 → +0
      score += Math.max(0, 5 - rank);

      // Sinyal ekstra (kurva / Marco)
