...
function getConfig(): Config {
  const mode = (process.env.STRATEGY_MODE || "balanced") as Mode;
  const base = {
...
export async function findOpportunities(config?: Config): Promise<{ token: Ingredient; amountCHEF: number }[]> {
  ...
  for (const ing of ingredients) priceMap.set(ing.id, parseFloat(ing.current_price_chef));
  const currentExposure = positions.reduce((sum, p) => sum + (priceMap.get(p.tokenId) || p.buyPriceCHEF) * p.quantityDecimal, 0);
  ...
  if (totalNeeded > remaining) amountPerToken = remaining / selected.length;
