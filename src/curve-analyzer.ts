...
export async function getBondingCurveSignals(tokenAddress: string): Promise<string[]> {
  ...
  // Harga akan segera naik (supply sudah ≥ 95% ke step berikutnya)
  if (analysis.isNearMilestone) signals.push("PRE_MILESTONE");

  // Lonjakan harga > 5% (lebih realistis daripada 30%)
  if (analysis.priceJumpPercent >= 5) signals.push("BIG_JUMP_AHEAD");

  // Token dengan supply tersisa sedikit, lebih mudah "dipompa"
  if (analysis.supplyNeededForNextStep < 500_000) signals.push("CHEAP_PUMP");
