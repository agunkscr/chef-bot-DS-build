...
async function sendWithBuffer(contract: ethers.Contract, method: string, args: any[]): Promise<ethers.TransactionReceipt> {
  ...
  const gasWithBuffer = (gasLimit * 120n) / 100n; // +20% buffer
...
export async function buyIngredient(
  ...
  try {
    // Langkah 1: Pastikan approval (dengan verifikasi receipt)
    await ensureApproval(amountCHEF);

    // Langkah 2: Hitung minTokens (slippage protection)
...
export async function sellIngredient(
  ...
  try {
    ...
    logTrade({ timestamp: new Date().toISOString(), action: "SELL", tokenAddress, symbol, amountCHEF: expectedReturn, tokensReceived: parseFloat(quantity), txHash: receipt.hash, success: true });
    return { success: true, txHash: receipt.hash };
  } catch (err: any) {
    console.error(`❌ SELL gagal ${symbol}:`, err.message);
    logTrade({ timestamp: new Date().toISOString(), action: "SELL", tokenAddress, symbol, amountCHEF: expectReturn, tokensReceived: parseFloat(quantity), txHash: "", success: false, error: err.message });
    return { success: false, error: err.message };
  }
}